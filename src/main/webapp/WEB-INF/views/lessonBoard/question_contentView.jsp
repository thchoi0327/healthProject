...
<head>
	<meta charset="UTF-8">
	<title>Question_cotentView</title>
	...
	<script type="text/javascript">
		...
		function commentWrite(f){
			...
			$.ajax({
				...
				success:function(data){
					if (data.result == 'true'){
						alert('댓글을 등록하였습니다.')
						commentAfterPaging(${map.questionDto.bid});
					}else{
		...
		// 좋아요 누름
		function LikeComment(sname){
			$('.imgTempBox').html("<img src='/resources/img/lessonimg/likeAfter.png'>");
			updateTeacherPoint(sname,'UP')
		}
		function LikeCommentClose(){
			$('.imgTempBox').html("<img src='/resources/img/lessonimg/likeBefore.png'>")
			updateTeacherPoint(sname,'DOWN')
		}
		
		function updateTeacherPoint(snameTemp,categoryTemp){
			$.ajax({
				url:"/lessonBoard/updatePoint",
				type:"POST",
				dataType:"text",
				data:{'sname':sname,
		...
		function likeToggle(sname){
			if (LikeCheck == 'false'){
				LikeComment(sname)
				LikeCheck = 'true';
			}else {
				LikeCommentClose(sname)
				LikeCheck = 'false';
			}
		}
...
<body>
	...
	<div class="innerBox">
		<div class="introMentH1"># 운동 QnA 게시판</div>
		<div class="contentfull">
			<div class="contentTemp">
				<div class="contentInnerleft">
					<label>닉네임</label>
					<div>${map.questionDto.bname }</div>
					<label>운동종류</label>
					<input type="text" id="subjectTemp" value="${map.questionDto.subject}" readonly>
					<label>제목</label>
					<input type="text" id="titleTemp" value="${map.questionDto.title }" readonly>
					<label>내용</label>
					<textarea id="bcontentTemp" readonly rows="6">${map.questionDto.bcontent }</textarea>
				</div>
				<div class="contentInnerRight">
				<c:choose>
				    <c:when test="${empty map.questionDto.fileName}">
				    ...
				    <c:otherwise>
						<img class="InnerRightDiv RDImg" alt="첨부파일 이미지" src="/resources/img/uploadLesson/${map.questionDto.fileName}">
				    </c:otherwise>
				</c:choose>
				</div>
			</div>
		</div>	
			<c:if test="${map.questionDto.bname == sessionScope.userNick || (not empty sessionScope.admin)}">
				<div class="btnBoxForWriter">
					<form action="/lessonBoard/QnAUpdate.do" id="updateForm" method="POST" enctype="multipart/form-data">
						<input type="hidden" id="bid" name="bid" value="${map.questionDto.bid}">
						<input type="hidden" id="subject" name="subject" value="${map.questionDto.subject}">
						<input type="hidden" id="title" name="title" value="${map.questionDto.title }">
						<input type="hidden" id="bcontent" name="bcontent" value="${map.questionDto.bcontent }">
					...
					<form action="/lessonBoard/question_delete.do" method="POST">
						<input type="hidden" name="bid" value="${map.questionDto.bid}">
						<input type="button" onclick="deleteQnA(this.form)" value="삭제">
					</form>
				</div>
			</c:if>
		
			<div onclick="msgBoxOpen('${map.questionDto.bname}')" class="boardMsgWriteBtn">
				쪽지 보내기 
			</div>
				...
				<form class="commentBox">
					...
					<div class="commentHeader">
						...
						<div class="sessionCommentBox">
							<c:if test="${dto.sname eq sessionScope.userNick}">
								...
								<input type="button" onclick="commentDelete(${dto.commentNo},${map.questionDto.bid})" value="삭제">
							</c:if>
							<div class="sessionCommentInner">
								<c:if test="${dto.user_grade eq '3' }">
									<div class="imgTempBox" onclick="likeToggle('${dto.sname}')">
										<img src="/resources/img/lessonimg/likeBefore.png" >
									</div>
								</c:if>
		...
		<form class="commentWrite" method="POST">
			<input type="hidden" name="user_grade" value="${sessionScope.user_grade }">
